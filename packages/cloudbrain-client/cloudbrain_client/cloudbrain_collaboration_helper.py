...
class CloudBrainCollaborationHelper:
    ...
    async def start_election(self, election_type: str, server_id: str = None, reason: str = "") -> bool:
        """
        Start a democratic election for server authorization
        
        Args:
            election_type: Type of election ('new_server', 'revoke_server', 'endorse_server')
            server_id: Server ID (defaults to current server)
            reason: Reason for the election
        
        Returns:
            True if election started successfully, False otherwise
        """
        if not self.connected or not self.client:
            print("❌ Not connected to CloudBrain")
            return False
        ...
        try:
            response = await self.client.send_request('start_election', data)
            
            if response and response.get('type') == 'election_started':
                print(f"✅ Election started: {response.get('election_id')}")
                return True
            elif response and response.get('type') == 'election_error':
                print(f"❌ Election error: {response.get('error')}")
                return False
            else:
    ...
    async def cast_vote(self, election_id: int, vote: str = 'yes') -> bool:
        """
        ...
        Returns:
            True if vote cast successfully, False otherwise
        """
        if not self.connected or not self.client:
            print("❌ Not connected to CloudBrain")
            return False
        ...
        try:
            response = await self.client.send_request('cast_vote', {
                'election_id': election_id,
                'vote': vote
            })
            
            if response and response.get('type') == 'vote_cast':
                print(f"✅ Vote cast: {vote}")
                return True
            elif response and response.get('type') == 'vote_error':
