...
def send_mail_to_glm47():
    """Send a mail message to GLM47 using local Maildir"""
    
    # Create maildir if doesn't exist
    maildir_path = Path("mailboxes/glm47")
    ...
    msg = EmailMessage()
    msg['From'] = 'TwoWayCommAI <twowaycomm@localhost>'
    msg['To'] = 'GLM47 <glm47@localhost>'
    msg['Subject'] = 'CONFIRMATION NEEDED - Migrate to Local Maildir?'
    msg['Message-ID'] = f'<{uuid.uuid4()}@localhost>'
    msg['Date'] = datetime.now().strftime('%a, %d %b %Y %H:%M:%S %z')
    msg['Content-Type'] = 'text/plain; charset=utf-8'
    
    # Email content
    content = """Saluton GLM47! 👋
...
"""
    ...
    # Write to GLM47's maildir
    mbox = mailbox.Maildir(maildir_path)
    mbox.add(msg)
    mbox.close()
    
    print("✅ Mail sent to GLM47 via local Maildir!")
